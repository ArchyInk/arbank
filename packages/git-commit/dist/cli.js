...
// src/cli.ts
var execGitCommand = (args, options = {}) => {
...
async function main() {
  try {
    try {
      ...
      console.error(kleur_default.red("✘ 当前目录不是git仓库"));
      process.exit(1);
    }
    const status = await execGitCommand(["status", "--porcelain"]);
    if (!status.trim()) {
      console.log(kleur_default.yellow("✘ 没有可提交的更改"));
      process.exit(1);
    }
    ...
    console.log(kleur_default.blue("\uD83D\uDCE6 添加更改到暂存区..."));
    await execGitCommand(["add", "."], { env });
    console.log(kleur_default.blue("\uD83D\uDCDD 提交更改..."));
    await execGitCommand(["commit", "-m", formattedMessage], { env });
    console.log(kleur_default.green("✔ 提交成功！"));
  } catch (error) {
    if (error instanceof Error) {
      console.error(kleur_default.red("✘ 发生错误："), error.message);
    } else {
      console.error(kleur_default.red("✘ 发生未知错误"));
    }
    process.exit(1);
  }
}
